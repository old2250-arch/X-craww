...
const BOT_TOKEN = "8219724688:AAGsL42CcOQ0ZixS_mJPR49OylYLXRm7hEg";
const OWNER_ID = "8375293596";
    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "WILZZDEV");
...
bot.command("start", (ctx) => {
  const teks = `( 弟 ) – Hi! || I'm Bot Apps Api, a Telegram bot built by wilzz using JavaScript technology. I was made to assist you, so use me wisely
...
bot.launch();
console.log(chalk.red(`╭─☐   wilzz x-craww
├─ ID Owner : ${OWNER_ID}
├─ Creatoe : @wilzzofficial
├─ Support : allah swt
├─ Status : Connected ✅
╰───────────────────`));
